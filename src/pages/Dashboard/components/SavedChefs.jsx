...
const SavedChefs = () => {
  ...
  const { data: favoritesRes, isLoading } = useGetFavoritesQuery(undefined, {
    skip: !userToken,
  });
    ...
    <div className="flex flex-col gap-6 md:gap-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-28 md:pb-0">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-500 font-medium text-sm md:text-base">
          Your curated collection of elite culinary talent.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {isLoading ? (
          ...
          savedChefs.map((fav) => {
            ...
            const price = profile.startingPricePerPerson
              ? `$${profile.startingPricePerPerson}`
              : "$100";

            return (
              <Card
                key={chefId || Math.random()}
                className="group overflow-hidden border-transparent bg-white shadow-sm hover:shadow-xl transition-all duration-500 rounded-[32px]"
              >
                <div
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-gray-400 font-medium max-w-[180px]">
              Add more elite talent to your private collection.
            </p>
          </div>
