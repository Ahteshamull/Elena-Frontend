...
export default function BrowseChefs() {
  ...
  const filteredChefs = allChefs.filter((user) => {
    ...
    if (filters.location && filters.location.trim() !== "") {
      const chefLocation =
        `${profile.city || ""} ${profile.country || ""} ${profile.travelRadiusLocation || ""}`.toLowerCase();
    ...
    if (filters.cuisines.length > 0) {
      ...
      const hasCuisine = filters.cuisines.some((c) =>
        chefCuisines.some((chefCuisine) =>
          chefCuisine.toLowerCase().includes(c.toLowerCase()),
        ),
      );
  ...
  const sortedChefs = [...filteredChefs].sort((a, b) => {
    ...
    if (sortBy === "PRICE: LOW TO HIGH") {
      return (
        (profileA.startingPricePerPerson || 0) -
        (profileB.startingPricePerPerson || 0)
      );
    }
    ...
    // HIGHEST RATED
    return (
      (parseFloat(b.averageRating) || 5) - (parseFloat(a.averageRating) || 5)
    );
  });
  ...
  return (
    <div className="pt-24 md:pt-32 pb-16 md:pb-24 max-w-7xl mx-auto px-4 sm:px-6 min-h-screen">
      ...
      <div className="mb-8 md:mb-16">
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif mb-3 md:mb-4 text-primary-900 leading-tight">
          The Culinary Collection
        </h1>
        <p className="text-gray-500 max-w-2xl text-base md:text-lg">
          Exquisite private dining experiences curated by world-renowned chefs,
      ...
      <div className="lg:hidden mb-4">
        ...
        >
          ...
          Filters{" "}
          {filters.cuisines.length > 0 && `(${filters.cuisines.length})`}
        </button>
      </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center mb-8 border-b border-gray-200 pb-4">
            <span className="text-sm font-semibold text-primary-900">
              <span className="font-bold">{sortedChefs.length}</span> Chefs
              available {filters.location ? `in ${filters.location}` : ""}
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {isLoading ? (
              ...
              <div className="col-span-1 md:col-span-2 text-center py-20 text-gray-500">
                No chefs found matching your criteria. Try adjusting your
                filters.
              </div>
            ) : (
              paginatedChefs.map((user) => {
                ...
                const image = rawImage
                  ? rawImage.startsWith("http")
                    ? rawImage
                    : `http://localhost:8005${rawImage}`
                  : "/b_1.png";

                const rating = Number(user.averageRating).toFixed(1) || "5.0";
                ...
                const nextAvailableDate =
                  availableDates.length > 0
                    ? new Date(
                        Math.min(...availableDates.map((d) => new Date(d))),
                ...
                return (
                  <Card
                    key={id}
                    className="group border-transparent shadow-none transition-all duration-300"
                  >
                    <Link
                      to={`/chef/${id}`}
                      className="block relative h-72 overflow-hidden rounded-t-2xl"
                    >
                      <img
                        src={image}
                        alt={name}
                    ...
                    <CardContent className="pt-6 px-4 bg-[#FAFAFA]">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-2xl font-serif font-medium capitalize">
                          {name}
                        </h3>
                      ...
                      <div className="flex flex-wrap gap-2 mb-8 mt-2">
                        {tags.slice(0, 4).map((tag) => (
                          <span
                            key={tag}
                            className="bg-gray-100 text-gray-600 text-xs font-medium px-3 py-1 rounded-full capitalize"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                        ...
                        <div className="flex gap-4 items-center">
                          <Link
                            to={`/chef/${id}`}
                            className="text-xs font-bold text-gray-500 hover:text-primary-900 uppercase tracking-widest transition-colors"
                          >
                            View Profile
                          </Link>
                          <Button
