...
import { useGetChefBookingsQuery, useUpdateBookingStatusMutation } from '../../../redux/api/bookingApi';
...
const ChefBookings = () => {
  ...
  const bookings = useMemo(() => {
    const sorted = [...fetchedBookings].sort((a, b) => 
      new Date(b.bookingDetails?.eventDate || 0) - new Date(a.bookingDetails?.eventDate || 0)
    );
    
    const searchFiltered = sorted.filter(booking => {
      const clientName = (booking.clientInfo?.userName || '').toLowerCase() || (`${booking.bookingDetails?.firstName || ''} ${booking.bookingDetails?.lastName || ''}`).trim().toLowerCase();
      const location = (booking.bookingDetails?.eventLocation || '').toLowerCase();
      const search = searchTerm.toLowerCase();
      return clientName.includes(search) || location.includes(search);
    });

    return {
      upcoming: searchFiltered.filter(b => ['pending', 'confirmed'].includes(b.bookingDetails?.status)),
      past: searchFiltered.filter(b => b.bookingDetails?.status === 'completed'),
      cancelled: searchFiltered.filter(b => ['cancelled', 'declined'].includes(b.bookingDetails?.status))
    };
      ...
      <div className="grid gap-4">
        {isLoading ? (
          ...
          bookings[activeTab].map((booking) => {
            ...
            const client = clientInfo.userName || `${booking.bookingDetails.firstName || ''} ${booking.bookingDetails.lastName || ''}`.trim() || "Client";
            const event = "Private Booking";
            const dateObj = new Date(booking.bookingDetails.eventDate);
            const dateStr = !isNaN(dateObj) ? dateObj.toLocaleDateString() : "TBD";
            ...
            const amount = booking.bookingDetails.totalAmount ? `$${booking.bookingDetails.totalAmount}` : "TBD";
            ...
            return (
            <Card key={bId} className="p-0 overflow-hidden border-gray-100 bg-white shadow-sm hover:shadow-md transition-all group">
              <div className="p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-6 w-full md:w-auto">
                  <div className="relative">
                    {image ? (
                      <img src={image} alt={client} className="w-20 h-20 rounded-2xl object-cover ring-2 ring-gray-50 group-hover:ring-accent/20 transition-all" />
                    ) : (
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                      <h3 className="text-lg font-bold text-primary-900">{client}</h3>
                      <Badge 
                        variant={
                    ...
                    <p className="text-sm font-serif italic text-gray-500">{event}</p>
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-1">
                      <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                        <Calendar size={14} className="text-accent" /> {dateStr}
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                        <Clock size={14} className="text-accent" /> {timeStr}
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                        <Users size={14} className="text-accent" /> {guests} Guests
                      </div>
                      <div className="flex items-center gap-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-gray-50">
                  ...
                  {status === 'pending' ? (
                    ...
                    <div className="flex items-center gap-2">
                      <Button 
                        onClick={() => navigate(`/chef-dashboard/bookings/${bId}`)}
