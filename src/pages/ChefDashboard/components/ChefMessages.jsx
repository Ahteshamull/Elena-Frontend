...
export default function ChefMessages() {
  ...
  const handleSendMessage = (textToSend) => {
    ...
    const receiverId =
      conv.participants.find((p) => String(p._id || p.id) !== currentUserId)
        ?._id ||
      conv.participants.find((p) => String(p._id || p.id) !== currentUserId)
        ?.id;
  ...
  const filteredChats = conversations
    ...
    .map((conv) => {
      ...
      return {
        ...
        clientName: name,
        clientImage: image,
        role: participant?.role === "chef" ? "Chef" : "Client",
        status: "online",
        unread: unreadCount,
  ...
  const suggestions = [
    {
      label: "Dietary Check",
      text: "Hello! Let me know if there are any specific dietary requirements or food allergies.",
    },
    {
      label: "Arrival Schedule",
      text: "Hi! What time should I arrive to set up the kitchen?",
    },
    {
      label: "Wine Pairings",
      text: "Would you like me to share some wine pairing options?",
    },
  ];
        ...
        {activeChat ? (
          ...
          >
            <div className="bg-white border-b border-gray-100 px-4 md:px-6 py-4 flex justify-between items-center z-10 shrink-0">
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <div>
                  <h3 className="font-serif text-lg text-primary-900 leading-none">
            ...
            <div
              ...
              className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 overscroll-contain"
            >
              {isLoadingMsgs ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8 text-gray-400 font-light text-xs gap-3">
                  ...
                  <span>
                    No messages yet. Send a note below to start planning!
                  </span>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1.5">
                        {msg.text && (
                          <div
                            className={cn(
                              "px-5 py-3.5 rounded-[1.5rem] text-sm leading-relaxed text-left",
                              !isMe
                        ...
                        <div
                          className={cn(
                            "text-[10px] text-gray-400 flex items-center gap-1 font-light",
                            !isMe ? "pl-2" : "justify-end pr-2",
                          )}
                        >
                          <span>
          ...
          >
            <div className="w-20 h-20 rounded-full bg-accent/5 border border-accent/10 flex items-center justify-center mb-6 shadow-sm shadow-accent/5">
              <Users size={36} className="text-accent animate-pulse" />
            </div>
            ...
            <p className="text-gray-400 text-sm max-w-sm leading-relaxed font-light">
              Select a client from the left sidebar to coordinate upcoming
              menus, check special dietary needs, and organize logistics for a
              flawless service.
            </p>
          </div>
        )}
