...
export default function ChefMessages() {
  ...
  const filteredChats = conversations.filter(conv => {
    ...
    const name = participant?.profile?.fullName || participant?.userName || participant?.name || "Client";
    return name.toLowerCase().includes(searchTerm.toLowerCase());
  }).map(conv => {
    const currentUserId = currentUser?._id || currentUser?.id;
    const participant = conv.participants.find(p => (p._id || p.id) !== currentUserId) || conv.participants[0];
    const profile = participant?.profile || {};
    const name = profile.fullName || profile.displayName || participant?.userName || participant?.name || "Client";
  ...
  const suggestions = [
    { label: "Dietary Check", text: "Hello! Let me know if there are any specific dietary requirements or food allergies." },
    { label: "Arrival Schedule", text: "Hi! What time should I arrive to set up the kitchen?" },
    { label: "Wine Pairings", text: "Would you like me to share some wine pairing options?" }
  ];
            ...
            <div className="flex items-center gap-2 md:gap-4">
              ...
              <div className="relative">
                <div className="w-11 h-11 rounded-full overflow-hidden border border-gray-100 bg-gray-50 flex items-center justify-center font-bold text-gray-400 uppercase text-lg">
                  {activeChat.clientImage && activeChat.clientImage !== "/b_1.png" ? (
